...
def init_db():
    def _write():
        ...
        try:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_attendance_person_date ON attendance(person_id, date);")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_people_embedding_key ON people(embedding_key);")
            conn.commit()
        finally:
            conn.close()
...
def get_attendance_snapshot(target_date):
    ...
    try:
        ...
        cur.execute(
            """
            SELECT p.id AS person_id, p.name, p.place, p.phone, a.last_seen
            FROM people p
        ...
        cur.execute(
            """
            SELECT p.id AS person_id, p.name, p.place, p.phone
            FROM people p
            LEFT JOIN attendance a ON a.person_id = p.id AND a.date = ?
            WHERE a.person_id IS NULL OR a.status = 'absent'
            ORDER BY p.name
            """,
            (target_date,),
        )
...
def delete_person_by_id(person_id):
    def _write():
        ...
        try:
            cur = conn.cursor()
            cur.execute("SELECT embedding_key FROM people WHERE id=?", (person_id,))
            row = cur.fetchone()
            if not row:
                return None
...
def update_person_details(person_id, new_name, new_place, new_phone):
    def _write():
        conn = get_connection()
        try:
            cur = conn.cursor()
            cur.execute("SELECT name, phone, embedding_key FROM people WHERE id=?", (person_id,))
            row = cur.fetchone()
            if not row:
                return None
            old_embedding_key = row["embedding_key"]
            new_embedding_key = f"{new_name.lower()}|{new_phone}"
            cur.execute(
                "UPDATE people SET name=?, place=?, phone=?, embedding_key=? WHERE id=?",
                (new_name, new_place, new_phone, new_embedding_key, person_id)
            )
            conn.commit()
            return old_embedding_key, new_embedding_key
        finally:
            conn.close()
    return with_db_retry(_write)


def delete_attendance_by_id(person_id, target_date):
    def _write():
        conn = get_connection()
        try:
            cur = conn.cursor()
            cur.execute("DELETE FROM attendance WHERE person_id=? AND date=?", (person_id, target_date))
            conn.commit()
        finally:
            conn.close()
    with_db_retry(_write)
...
def update_attendance_status_by_id(person_id, target_date, status):
    def _write():
        now = datetime.now().isoformat(timespec="seconds")
        last_seen = now if status == "present" else "-"
        conn = get_connection()
        try:
            cur = conn.cursor()
            cur.execute(
                ...
                """,
                (person_id, target_date, status, last_seen)
            )
            conn.commit()
        finally:
            conn.close()
...
def clear_attendance_for_date(target_date):
    def _write():
        conn = get_connection()
        try:
            cur = conn.cursor()
            cur.execute("DELETE FROM attendance WHERE date=?", (target_date,))
            conn.commit()
        finally:
            conn.close()
    with_db_retry(_write)
...
def get_attendance_export_rows(target_date):
    conn = get_connection()
    try:
        cur = conn.cursor()
        cur.execute(
